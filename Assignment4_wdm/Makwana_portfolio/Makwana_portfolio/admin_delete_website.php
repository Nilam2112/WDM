...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  ...
  if (empty($_POST["title"])) {
    ...
    if(!preg_match("/^[A-Za-z]*$/",$title))
      {
                $titleErr = "Only letters allowed for title ";
    }
...
$sql = "DELETE FROM website WHERE title='$_POST[title]'";

echo "deleted";
echo $conn->error;
if($conn->query($sql) === TRUE)
{
  header("location: admin_delete_website.php");
}
else
{
...
<body id="wrapper">
     ...
     <!-- <form name="SignForm" action="" onsubmit="return ValidateSign()" method="post">-->
                ...
                <p style="font-family: Roboto; font-size: 0.70em; color: #777777; margin-left:16px; margin-top: 10px;">Admin can delete  portfolio information: </p>
                <hr style="color: #FFFFFF;">

            <label  for="title" style="color: #777777; font-size: 0.70em; font-family: Roboto; margin-left:16px;margin-top: 20px;">title Name:</label><br/>
            <input type="text" id="title" name="title"  placeholder="Enter title"   title="please Enter a valid name"  style="width:456px;height:36px;color:#777777;font-size:0.70em;font-family:Roboto;margin-left:13px;">
